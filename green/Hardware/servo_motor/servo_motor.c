...
void Spinnig_Level(int diff)
{
	if(diff<0)
	{
		Oc_Lp[0]=Oc_L=(Oc_L+diff)<500?500:(Oc_L+diff);
	}
	else if(diff>0)
	{
		Oc_Lp[0]=Oc_L=(Oc_L+diff)>1000?1000:(Oc_L+diff);	
	}
...
void Spinnig_Vert(int diff)
{
	if(diff<0)
	{
		Oc_Vp[0]=Oc_V=(Oc_V+diff)<500?500:(Oc_V+diff);
	}
	else if(diff>0)
	{
		Oc_Vp[0]=Oc_V=(Oc_V+diff)>1000?1000:(Oc_V+diff);	
	}
...
void Reach_Pos_CL(int Target_X,int Target_Y,int Reach_Pos_CL_MODE)
{
	...
	while(Reach_Pos_CL_Stop_Sign)
	{
		...
		if(near(Target_X,Target_Y)<=20)
		{
			Beep_Times(100,1,NORMAL_MODE);
			break;
		}
...
int Get_Point_5(void)
{
	...
	Timer_delay_ms(200);
	if(USART_RX_INFO[0]=='i'&&USART_RX_INFO[4]=='j' && sign)
	{
