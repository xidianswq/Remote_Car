...
{
	if(SERVO090+diff<=SERVO000)
	{
		Off_L=SERVO000;
	}
	else if(SERVO090+diff>=SERVO180)
	{
		Off_L=SERVO180;	
	}
	else
...
void Spinnig_Vert(int diff)
{
	if(diff<0)
	{
		Off_V=(Off_V+diff)<SERVO000?SERVO000:(Off_V+diff);
	}
	else if(diff>0)
	{
		Off_V=(Off_V+diff)>SERVO180?SERVO180:(Off_V+diff);	
	}
...
*********************************************************/
#include "usart.h"
#define LENGTH 640	//坐标系X上限
#define HEIGHT 480	//坐标系Y上限
